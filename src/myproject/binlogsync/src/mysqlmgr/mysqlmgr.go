...
func (mgr *MysqlMgr) InsertBackupUploadDB(info protocal.DbInfo, tablename string) int {
	insertsql := "INSERT INTO " + "live_master." + tablename + "(task_id,file_name," +
	...
	// 源文件默认为0，备份文件该值为1
	_, err = stmtIns.Exec(info.TaskId, info.FileName, info.FileType, info.FileSize, info.Domain,
...
func (mgr *MysqlMgr) SelectFailedInfo(tablename string) (int, []protocal.DbInfo) {
	...
	for rows.Next() {
		err := rows.Scan(&id, &fid, &taskid, &filename, &filetype, &filesize,
