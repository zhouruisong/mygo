...
func main() {
	...
	t := logger.GetLogger(cfg.LogPath, "tair_agent")
	...
	pTair := tair.NewTairClient(cfg.TairServer, cfg.TairClient, t)
	if pTair == nil {
		l.Errorf("NewTairClient fail")
		return
	}

	pCentre := centre.NewClusterMgr(pFdfsmgr, pTair, cfg.MysqlAgent, cfg.FdfsAgent, cfg.TairAgent, d)
	if pCentre == nil {
