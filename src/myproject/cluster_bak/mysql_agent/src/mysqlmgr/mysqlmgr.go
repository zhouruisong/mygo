...
//func (mgr *MysqlMgr) LoadStreamInfos(beginIndex int, tablename string) ([]StreamInfo, int) {
...
func (mgr *MysqlMgr) InsertMultiStreamInfos(info []StreamInfo, tablename string) int {
	datalen := len(info)
	if datalen == 0 {
...
func (mgr *MysqlMgr) DeleteMultiStreamInfos(info []StreamInfo, tablename string) int {
	//	datalen := len(info)
	//	if datalen == 0 {
	//		mgr.Logger.Errorf("datalen = 0")
	//		return -1
